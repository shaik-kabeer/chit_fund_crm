...
function AddSeatForm({ groupId }: { groupId: string }) {
  ...
  const { data: customers } = useQuery({
    queryKey: ['admin-customers-pick', customerSearch],
    queryFn: () => api.get<any>(`/customers?page=1&limit=500&search=${customerSearch}`),
    enabled: open,
  });
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <div className="sm:col-span-2 space-y-2">
          ...
          <select
            ...
            size={Math.min((customers?.data?.length || 0) + 1, 8)}
            className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Select customer ({customers?.data?.length || 0} found)</option>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Link href="/admin/groups" className="hover:text-blue-600 transition">← Groups</Link>
        <span>/</span>
        <span className="text-gray-800 font-medium">{group.product?.name} — {group.groupNumber}</span>
      </div>
        ...
        <div className="flex justify-between items-start flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              {group.currentMonth > 0 && months.length > 0
                ? `Current: ${months.find((x: any) => x.monthNumber === group.currentMonth)?.monthLabel || `Month ${group.currentMonth}`}`
          ...
          <form onSubmit={(event) => { event.preventDefault(); editGroupMutation.mutate(); }}
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              ...
              <label className="text-sm">Start date
                <input type="date" value={groupForm.startDate}
        ...
        <div className="space-y-4">
          {canManage && (
            <div className="flex justify-end gap-2">
              ...
              >
                {remindUnpaidMutation.isPending ? 'Sending...' : `Notify Unpaid (${months.find((x: any) => x.monthNumber === activeMonthView)?.monthLabel || `Month ${activeMonthView}`})`}
              </button>
            ...
            {Array.from({ length: tenure }, (_, i) => i + 1).map((m) => {
              ...
              return (
                <button key={m} onClick={() => setSelectedMonth(m)}
                  className={`min-w-[4.5rem] px-2 h-10 rounded-lg text-xs font-medium border transition ${
                  ...
                  title={info?.lifted ? `Lifted by ${info.lifted.name}` : info?.monthLabel || `Month ${m}`}
                >
                  {info?.monthLabel || `M${m}`}
                </button>
              );
          ...
          {monthView && (
            <div className="bg-white rounded-xl border p-6 space-y-5">
              <div className="flex flex-wrap justify-between gap-4">
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">{monthView.monthLabel || `Month ${monthView.monthNumber}`}</h2>
                  <p className="text-sm text-gray-500 mt-1">
                    Month {monthView.monthNumber} · Scheduled lift payout: <strong>{formatCurrency(monthView.scheduledPayoutPaise)}</strong>
                    {' · '}
            ...
            <tbody>
              ...
              {[...seated].sort((a: any, b: any) => (a.ticketNumber || 0) - (b.ticketNumber || 0)).map((m: any) => (
                <tr key={m.id} className="border-b last:border-0">
                  ...
                  <td className="py-3 text-gray-500">{m.prizedMonth ? (months.find((x: any) => x.monthNumber === m.prizedMonth)?.monthLabel || `Month ${m.prizedMonth}`) : '—'}</td>
